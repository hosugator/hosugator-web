...
export default function Sidebar() {
  ...
  return (
    <aside 
      ...
      className={`fixed left-0 top-0 h-screen z-[100] transition-all duration-300 ease-in-out border-r border-slate-100 bg-white ${
        isHovered ? "w-64 shadow-2xl" : "w-16"
      }`}
    >
      {/* p-4로 유지하여 H. 로고가 본문 시작 라인과 비슷하게 위치하도록 조정 */}
          ...
          <nav className="flex flex-col gap-10 text-sm font-bold tracking-[0.2em] uppercase">
            {['About', 'Experience', 'Skills'].map((item) => (
              <Link 
                ...
                className="hover:text-[#13ecda] transition-colors text-slate-900 flex items-center gap-6 ml-1"
              >
