      ...
      <div className="max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {t.insights.items.map((item, index) => (
            <div
              key={index}
              className={`group relative border border-slate-100 rounded-2xl p-8 hover:border-[#13ecda]/40 hover:shadow-lg hover:shadow-[#13ecda]/5 transition-all duration-300 ${
                index >= 3 ? 'md:col-span-1' : ''
              } ${t.insights.items.length === 5 && index === 3 ? 'md:col-start-1' : ''}`}
            >
              <div className="text-xs font-mono font-bold text-slate-300 mb-6 group-hover:text-[#13ecda] transition-colors">
                {item.number}
              </div>

              <h4 className="text-2xl font-black text-slate-900 mb-4 group-hover:text-[#13ecda] transition-colors">
                {item.title}
              </h4>
              ...
              <span className="inline-block px-3 py-1 text-xs font-bold bg-[#13ecda]/10 text-[#13ecda] rounded-full">
                {item.project}
              </span>
            </div>
          ))}
