  ...
  return (
    <section id="knowledges" className="py-12 md:py-32 border-t border-slate-100 relative text-slate-900">
      <div className="w-full">
        ...
        <div 
          ref={containerRef} 
          className="w-full h-[550px] md:h-[700px] border border-slate-100 rounded-xl md:rounded-3xl overflow-hidden bg-slate-50/30 relative"
        >
          <ForceGraph2D
            ref={fgRef}
            ...
            nodeCanvasObject={(node, ctx, globalScale) => {
              ...
              const fontSize = isRoot ? 17 / globalScale : 11 / globalScale;
              ctx.font = `${isRoot ? '900' : '400'} ${fontSize}px Sans-Serif`;
              const radius = isRoot ? 14 : 4;
              ctx.beginPath();
              ctx.arc(node.x!, node.y!, radius, 0, 2 * Math.PI, false);
              ctx.fillStyle = isRoot ? '#13ecda' : '#cbd5e1';
              ctx.fill();
              if (globalScale > 0.35 || isRoot) {
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.fillStyle = isRoot ? '#13ecda' : '#64748b';
          ...
          {/* 버튼 위치 수정: scale-0.8 대응을 위해 right 값을 키움 */}
          <div className="absolute top-4 right-[22%] md:right-4 flex items-center gap-2 z-20">
            <button 
              onClick={(e) => {
                e.stopPropagation();
                fitToCenter(800);
