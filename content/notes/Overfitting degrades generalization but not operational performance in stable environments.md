...
- val_loss도 낮음 — 분포 D 내에서는 일반화됨. 분포 D만으로는 환경 종속성을 판단하기 어려움.
...
### 환경 종속성은 D' 테스트셋 성능으로만 판단할 수 있다

val_loss는 분포 D 내부 지표다. D'(다른 라인, 카메라, 조명)로의 전이 실패는 val_loss로 감지되지 않는다.

```
고전적 과적합 진단: random split val_loss  (높으면 → D 내에서도 일반화 실패)
환경 종속성 진단:   D' 테스트셋 운영 지표 (낮으면 → D'로 전이 실패)
```

D'가 없다 = 환경 종속성 여부를 모른다. "운영 환경이 D로 고정됐다"는 전제가 성립할 때만 환경 종속성이 문제 없다는 판단이 유효하다.
