...
```kotlin
// MVVM — 관계 선언 후 구독
val viewModel: InferenceViewModel by activityViewModels()
viewModel.result.collect { render(it) }
```
...
```tsx
// React — 그냥 호출
const { result } = useInference();
```

### MVVM의 구독 모델은 화면 단위에서 강하고 조각 단위에서 약하다
...
호출형은 조각 단위 자유도를 얻는 대신 **상태 공유 복잡도**를 잃는다. MVVM은 ViewModel 스코프로 화면 내 상태 공유가 명확하다. React는 같은 Hook을 두 컴포넌트가 호출하면 다른 인스턴스가 된다 — 공유하려면 상태를 부모로 올리거나 Redux 같은 전역 store를 추가해야 한다.

```
MVVM  — 관계 명확, 공유 자연스러움 / 조각 단위 조합 비용 있음
React — 조합 자유로움 / 공유 상태 설계를 매번 직접 결정해야 함
```
